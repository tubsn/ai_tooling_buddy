	...
	public function request(
		...
		if ($jsonPayload === false) {
			throw new \RuntimeException(
				"JSON encode payload failed: " . json_last_error_msg()
			);
		}
		...
		$options = [
			...
			CURLOPT_ENCODING => "",
			CURLOPT_USERAGENT => "OpenAI-PHP-ResponsesClient",
		...
		if ($onChunk) {
			...
			$options[CURLOPT_WRITEFUNCTION] = function (
				$curlHandleInner,
				$incomingData
			) use ($onChunk) {
				...
				while (
					($newlinePos = strpos($this->sseBuffer, "\n")) !== false
				) {
					$line = trim(substr($this->sseBuffer, 0, $newlinePos));
					$this->sseBuffer = substr(
						$this->sseBuffer,
						$newlinePos + 1
					);
		...
		if ($onChunk) {
			if ($curlErrorCode !== 0) {
				throw new \RuntimeException(
					"Stream transport error: " . $curlErrorMessage
				);
			}
			if ($httpStatus >= 400) {
				$decodedError = json_decode($this->rawResponseBody, true);
				$apiMessage =
					$decodedError["error"]["message"] ?? "Unknown API error";
				throw new \RuntimeException(
					"Stream HTTP error: " . $httpStatus . " - " . $apiMessage
				);
			}
			return [];
		}
		...
		if ($curlErrorCode !== 0) {
			throw new \RuntimeException(
				"Transport error: " . $curlErrorMessage
			);
		}
		...
		if ($httpStatus >= 400) {
			...
			throw new \RuntimeException(
				"HTTP " . $httpStatus . ": " . $apiMessage
			);
		}
		if (isset($decoded["error"])) {
			throw new \RuntimeException(
				"API error: " . ($decoded["error"]["message"] ?? "unknown")
			);
		}
